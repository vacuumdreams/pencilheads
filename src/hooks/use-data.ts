...
const toEvent = (d: any): Event => ({
  ...
  attendance: Object.keys(d.attendance).reduce<Event['attendance']>((acc, key) => ({
    ...acc,
    [key]: {
      ...d.attendance[key],
...
export function useSpaceCollection(p?: HookParams): [Record<string, Space> | undefined, boolean, string | null] {
...
export function useEventCount(p?: HookParams & { spaceId?: string }): [number, boolean, string | null] {
  ...
  const [error, setError] = React.useState<FirestoreError | undefined>(undefined)
  const spaceId = useSpaceId()
  const [user] = useAuthState(auth)

  if (!user) throw new Error('Must be authorized to query events.')
  if (!p?.spaceId && !spaceId) throw new Error('Missing space id, cannot query events.')
